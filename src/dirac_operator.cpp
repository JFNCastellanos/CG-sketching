	...
	for(int x = 1; x<=width_x; x++){
		for(int t = 1; t<=width_t; t++){
			...
			Dphi.val[2*n] = (m0 + 2) * phi.val[2*n] - 0.5 * ( 
				...
				+	U.val[2*n+1]  					* (phi.val[2*rpb[2*n+1]] + I_number * phi.val[2*rpb[2*n+1]+1])
				+ std::conj(U.val[2*lpb[2*n]]) 		* lsign[2*n] 	* (phi.val[2*lpb[2*n]] + phi.val[2*lpb[2*n]+1])
				+ std::conj(U.val[2*lpb[2*n+1]+1]) 	  				*  (phi.val[2*lpb[2*n+1]] - I_number*phi.val[2*lpb[2*n+1]+1])
			);
			//mu = 1
			Dphi.val[2*n+1] = (m0 + 2) * phi.val[2*n+1] - 0.5 * ( 
					U.val[2*n] 	 * rsign[2*n] 		* (-phi.val[2*rpb[2*n]] + phi.val[2*rpb[2*n]+1])
				+	U.val[2*n+1] 					* (-I_number*phi.val[2*rpb[2*n+1]] + phi.val[2*rpb[2*n+1]+1])
				+ std::conj(U.val[2*lpb[2*n]]) 		* lsign[2*n] 	* (phi.val[2*lpb[2*n]] + phi.val[2*lpb[2*n]+1])
				+ std::conj(U.val[2*lpb[2*n+1]+1]) 					* (I_number*phi.val[2*lpb[2*n+1]] + phi.val[2*lpb[2*n+1]+1])
			);
		}
	}
